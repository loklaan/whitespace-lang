...
var asmWithValueParam = function() {
  return asmObject(this.labels, this.mnemoCode, this.param.value, null)
}

var asmWithLabelParam = function() {
  return asmObject(this.labels, this.mnemoCode, null, this.param.token)
}
...
var ws = {
    ...
    var self = {
      ...
      runProgram: function(program) {
        try {
          ...
          while (
            this.running &&
            this.register.IP < program.programStack.length
          ) {
  ...
  compile: function(fullSource) {
    ...
    var tokenizer = new WSUtil.StrArr(fullSource)
  ...
  programBuilder: function(fullSource, master) {
    ...
    builder.getAsmSrc = function() {
      var src = []
      var asm = this.getAsm()
      var labler = new WSUtil.labelTransformer(function(n, label) {
        return 'label_' + n
      })
      for (var i in asm) {
        var ln = asm[i]
        ...
        if (ln.param.label != null) {
          instrStr +=
            ' ' +
            (this.asmLabels[ln.param.label] || labler.getLabel(ln.param.label))
        }
